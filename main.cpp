...
struct queue{
    ...
    void print(){
        ...
        // Check if front has been changed by reversal since acquiring lock on muxtex
    ...
    void removeItem(int identifier){
        ...
        struct node* nexttemp;
        std::unique_lock<std::mutex> outputlock(outputmutex, std::defer_lock);
        std::unique_lock<std::mutex> sizelock(this->size_mutex, std::defer_lock); // Lock now
        std::unique_lock<std::mutex> lock_temp_node(front->m); // Lock now
        while (lock_temp_node.mutex() != &(front->m)){
            lock_temp_node.unlock();
            lock_temp_node= std::unique_lock<std::mutex>(front->m);
        ...
        if (temp == front){
            ...
            // If temp != front, then either printing has it locked (don't care because it doesn't change value of it)
            // Or reversal has a lock on it, but it cannot finish before removal has finished, so it won't have changed its value.
            front = nexttemp;
            if ((nexttemp->isNull)){
                // Only one node in the queue --> front == back, so back is locked too
                back = front;
            }
        } else if (temp == back){
...
void questionE(struct queue *queue){
    ...
    while (queue->size > 0){
        sizelock.unlock(); // Unlock here so other threads can START before final node removed, BUT they will NOT re-enter while loop after removal is finished
        usleep(200000); // Wait 0.2s = 200ms = 200 000 microseconds
        int position = rand()%queue->size;
...
int main(int argc, const char * argv[]) {
    ...
    questionB(q,chars);
    q->print();
    std::thread t1(questionC, q); // queue is passed by reference in this instance
    t1.detach(); // Make thread run in the background
    std::thread t2(questionD, q); // queue is passed by reference in this instance
    t2.detach(); // Make thread run in the background
    std::thread t3(questionE, q); // queue is passed by reference in this instance
    t3.detach(); // Make thread run in the background
    return 0;
}
